...
def load_garbage_data():
    try:
        # Excelファイルの読み込みを試みる
        df = pd.read_excel("shinjuku_garbage_schedule_2026.xlsx", sheet_name='収集日一覧')
...
def load_shinjuku_csv():
    try:
        return pd.read_csv('shinjuku_nann.csv', encoding='cp932')
    except Exception as e:
        return pd.DataFrame({
...
def load_model():
    try:
        if not MODEL_ENABLED:
            return None, ["可燃ごみ (Burnable Waste)", "不燃ごみ (Non-burnable Waste)"]
        ...
        try:
            ...
            labels = ["可燃ごみ (Burnable Waste) ✨", "不燃ごみ (Non-burnable Waste) 🛡️"]
        return interpreter, labels
    except Exception as e:
        return None, ["可燃ごみ (Burnable Waste) ✨", "不燃ごみ (Non-burnable Waste) 🛡️"]
...
# --- [画像分別カメラ] ---
    ...
    if image:
        ...
        if interpreter:
            ...
            predicted_label = labels[int(highest_match_index)].strip()
            predicted_confidence = float(prediction[0][highest_match_index]) * 100
            ...
            with st.expander(lang["all_results"]):
                for i, label in enumerate(labels):
                    conf = float(prediction[0][i]) * 100
                    status = "✅" if i == highest_match_index else "▫️"
                    st.write(f"{status} {label.strip()} ({conf:.2f}%)")
                    st.progress(max(0.0, min(float(prediction[0][i]), 1.0)))

            # 写真の下に、モデルが判定した品目の詳しい分別情報を表示
            show_waste_details(predicted_label)
        else:
            st.warning(lang["demo_warning"])
            st.write("✅ ペットボトル/pet bottle (95.00%)")
            st.progress(0.95)
            show_waste_details("ペットボトル/pet bottle")
